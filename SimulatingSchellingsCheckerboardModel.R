...
while(someoneuncontent(board)){
  ...
  # select mover
    ...
    # version with movement to places where the mover will be content
    ...
    # version with movement to anywhere
    aux_spotsavailable <- emptyspots
    # relocate mover
    if(sum(aux_spotsavailable) == 0){
      print("No moves possible.")
      no_move_counter <- no_move_counter + 1
    }else{
      aux_next_pos_ind <- sample(nrow(aux_spotsavailable), 1)
    ...
    tracked_avg_similarity <- c(tracked_avg_similarity, avg_similarity(board))
...
nb_report(board)
plot(1:length(tracked_avg_similarity), tracked_avg_similarity)

length(tracked_avg_similarity)
